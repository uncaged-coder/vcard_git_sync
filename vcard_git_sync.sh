...
# contact_dir = /data/perso/contacts
# vcardtools = /data/desk/vcardtools/vcardtools.py
...
CONFIG_FILE="$HOME/.config/vcard_git_sync/config.ini"
...
VCARDTOOLS=$(get_config_value "DEFAULT" "vcardtools")
OUT_DIR="${WORK_DIR}/out"
MERGE_ATTR="--match-attributes email --match-attributes names --match-attributes tel"
VCARDTOOLS_OPTIONS="-e .vcf --no-space-in-filename --force-lower-case-in-filename"
...
import_contacts() {
  ...
  echo "python3 ${VCARDTOOLS} ${VCARDTOOLS_OPTIONS} --merge ${MERGE_ATTR} ${OUT_DIR} ${CURRENT_MERGED_VCF} ${IMPORTED_MERGED_VCF}"
  python3 ${VCARDTOOLS} ${VCARDTOOLS_OPTIONS} --merge ${MERGE_ATTR} ${OUT_DIR} ${CURRENT_MERGED_VCF} ${IMPORTED_MERGED_VCF}
